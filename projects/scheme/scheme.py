...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    if scheme_eval(expressions.first, env):
        return expressions.second.first
    elif expressions.second.second != nil:
        return expressions.second.second.first
    else:
        return okay
    # END Question 13
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if type(target.first) == int:
            raise SchemeError
        New_frame.define(target.first, scheme_eval(target.second.first, env))
...
def check_formals(formals):
    ...
    temp = []
    while formals is not nil:
        if scheme_symbolp(formals.first) and formals.first not in temp:
            temp += formals.first
            formals = formals.second
        else:
            raise SchemeError
