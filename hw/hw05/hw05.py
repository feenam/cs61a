...
def height(t):
    ...
    if is_leaf(t):
        ...
        return 1 + max([height(subtree) for subtree in subtrees(t)])
...
def sprout_leaves(t, vals):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        return tree(entry(t), [tree(val) for val in vals])
    else:
        return tree(entry(t), [sprout_leaves(subtree, vals) for subtree in subtrees(t)])
