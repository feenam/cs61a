...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    find_c = [find_closest(restaurant_location(r), centroids) for r in restaurants]
    return group_by_first(zip(find_c, restaurants))
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lat_total, lon_total = [], []
    for lat, lon in [restaurant_location(r) for r in cluster]:
        lat_total += [lat]
        lon_total += [lon]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_x = mean(xs)
    mean_y = mean(ys)
    S_xx = sum([square(x - mean_x) for x in xs])
    S_yy = sum([square(y - mean_y) for y in ys])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for place in restaurants:
        if not place in reviewed:
            all_ratings[restaurant_name(place)] = predictor(place)
        else:
            all_ratings[restaurant_name(place)] = user_rating(user, restaurant_name(place))
    return all_ratings
    # END Question 9
